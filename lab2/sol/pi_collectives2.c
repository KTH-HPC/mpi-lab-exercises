 ...
 * Update 2023-08-08 Changed to illustrate mpi_gather. Martin Karp
 **********************************************************************/
#include "mpi.h"
#include <stdio.h>
#include <stdlib.h>
...
int main (int argc, char *argv[])
{
  ...
  int taskid,        /* task ID - also used as seed number */
    ...
    i,
    j;
  double * gather_array;   /* array to gather values in */
  MPI_Status status;
  ...
  if (taskid == MASTER) {
    ...
    gather_array = (double*) malloc(numtasks * sizeof(double));
  }
  ...
  for (i = 0; i < ROUNDS; i++) {
    ...
    rc = MPI_Gather(&homepi, 1, MPI_DOUBLE, gather_array, 
                    1,MPI_DOUBLE, MASTER, MPI_COMM_WORLD);


    if (rc != MPI_SUCCESS)
      printf("%d: failure on mpi_gather\n", taskid);
    
    /* Master computes average for this iteration and all iterations */
    if (taskid == MASTER) {
      pisum = 0.0;
      for ( j = 0; j < numtasks; j++)
          pisum += gather_array[j];
      pi = pisum/numtasks;
